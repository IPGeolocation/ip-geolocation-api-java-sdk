...
public class IPGeolocationAPI {
  ...
  public Map<String, Object> getGeolocation() {
    Map<String, Object> apiResponse =
        callAPIEndpoint("ipgeo", "apiKey=" + apiKey, "GET", Optional.empty());
  ...
  public Map<String, Object> getGeolocation(GeolocationParams params) {
    Map<String, Object> apiResponse =
        callAPIEndpoint("ipgeo", buildGeolocationUrlParams(params), "GET", Optional.empty());
  ...
  public Map<String, Object> getBulkGeolocation(GeolocationParams params) {
    String url =
        "https://api.ipgeolocation.io/ipgeo-bulk" + "?" + buildGeolocationUrlParams(params);
    List<Map<String, Object>> apiResponseList =
        callBulkAPIEndpoint(url, Optional.of(new JSONObject().put("ips", params.getIPAddresses())));
  ...
  public Map<String, Object> getTimezone() {
    Map<String, Object> apiResponse =
        callAPIEndpoint("timezone", "apiKey=" + apiKey, "GET", Optional.empty());
  ...
  public Map<String, Object> getTimezone(TimezoneParams params) {
    Map<String, Object> apiResponse =
        callAPIEndpoint("timezone", buildTimezoneUrlParams(params), "GET", Optional.empty());
    ...
    if (isNullOrEmpty(uaString)) {
      ...
      apiResponse =
          callAPIEndpoint(
              ...
              "apiKey=" + apiKey,
              "POST",
              Optional.of(new JSONObject().put("uaString", uaString)));
    }
  ...
  public Map<String, Object> getBulkUserAgent(List<String> uaStrings) {
    ...
    List<Map<String, Object>> apiResponseList =
        callBulkAPIEndpoint(url, Optional.of(new JSONObject().put("uaStrings", uaStrings)));
  ...
  private Map<String, Object> prepareBulkResponseForUser(
      List<Map<String, Object>> apiResponseList, String type) {
    ...
    if (httpStatus == 200 && type.equals("geolocation")) {
      response.put(
          "response", apiResponseList.stream().map(Geolocation::new).collect(Collectors.toList()));
    } else if (httpStatus == 200 && type.equals("useragent")) {
      response.put(
          "response", apiResponseList.stream().map(UserAgent::new).collect(Collectors.toList()));
    }
  ...
  private Map<String, Object> callAPIEndpoint(
      String endpoint, String urlParams, String requestType, Optional<JSONObject> requestBody) {
  ...
  private List<Map<String, Object>> callBulkAPIEndpoint(
      String url, Optional<JSONObject> requestBody) {
    int responseCode;
    String jsonString;
  ...
  private Map<String, String> getResponseMap(
      String url, String requestType, Optional<JSONObject> requestBody) {
  ...
  private Map<String, String> parseConnectionResponse(HttpURLConnection connection) {
    ...
    try {
      ...
      if (isNullOrEmpty(jsonString)) {
        ...
        responseMap.put(
            "json", "{\"message\":\"Could not get any response from ipgeolocation.io API.\"}");
      } else {
